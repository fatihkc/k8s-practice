...
resource "aws_security_group" "ingress_nodeport" {
  name        = "${local.cluster_name}-ingress-nodeport"
  description = "Allow incoming 30000 from outside the cluster"
  vpc_id      = var.vpc_id
  tags        = local.tags
  ingress {
    protocol    = "tcp"
...
resource "aws_instance" "workers" {
  ...
  vpc_security_group_ids = [
    ...
    aws_security_group.ingress_internal.id,
    aws_security_group.ingress_ssh.id,
    aws_security_group.ingress_nodeport.id
  ]
